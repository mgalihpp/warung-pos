...
import {
  useProducts,
  type BarangPageData,
} from "@/features/barang/hooks/use-barang-queries"

export function TambahBarangPageClient({
  initialData,
}: {
  initialData: BarangPageData
}) {
...
function BarangFormPageSkeleton() {
  return (
    <>
      ...
      <div className="hidden min-w-0 flex-col gap-4 p-4 lg:flex lg:gap-6 lg:p-6">
        ...
        <div className="flex gap-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="h-20 flex-1 animate-pulse rounded-xl bg-muted"
            />
          ))}
        </div>
